...
contract KTTYStaking is AccessControl, ReentrancyGuard, Pausable {
    ...
    // Events
    event TierCreated(uint256 indexed tierId, string name, uint256 minStake, uint256 lockupPeriod, uint256 apy);
    event TierUpdated(uint256 indexed tierId, string name, uint256 minStake, uint256 lockupPeriod, uint256 apy, bool isActive);
    event TierRewardTokenAdded(uint256 indexed tierId, address indexed tokenAddress);
    event TierRewardTokenRemoved(uint256 indexed tierId, address indexed tokenAddress);
    
    event RewardTokenRegistered(address indexed tokenAddress, string symbol, uint256 rewardRate);
    event RewardTokenUpdated(address indexed tokenAddress, uint256 rewardRate, bool isActive);
    
    event Staked(uint256 indexed stakeId, address indexed owner, uint256 amount, uint256 tierId, uint256 startTime, uint256 endTime);
    event StakeWithdrawn(uint256 indexed stakeId, address indexed owner, uint256 amount);
    event RewardClaimed(uint256 indexed stakeId, address indexed owner, address indexed token, uint256 amount);
    ...
    function pause() external {
        require(hasRole(ADMIN_ROLE, msg.sender) || hasRole(EMERGENCY_ROLE, msg.sender), "Caller must have admin or emergency role");
        _pause();
     ...
     */
    function addRewardTokenToTier(uint256 tierId, address tokenAddress) 
        external 
        onlyRole(TIER_MANAGER_ROLE) 
    {
        if (tiers[tierId].minStake == 0) revert TierDoesNotExist();
        if (rewardTokens[tokenAddress].tokenAddress == address(0)) revert TokenNotRegistered();
        
        // Check if the token is already in the tier
        for (uint256 i = 0; i < tiers[tierId].rewardTokens.length; i++) {
            if (tiers[tierId].rewardTokens[i] == tokenAddress) revert DuplicateRewardToken();
        }
     ...
     */
    function removeRewardTokenFromTier(uint256 tierId, address tokenAddress) 
        external 
        onlyRole(TIER_MANAGER_ROLE) 
    {
        if (tiers[tierId].minStake == 0) revert TierDoesNotExist();
        ...
        if (tokenIndex != tiers[tierId].rewardTokens.length - 1) {
            tiers[tierId].rewardTokens[tokenIndex] = tiers[tierId].rewardTokens[tiers[tierId].rewardTokens.length - 1];
        }
     ...
     */
    function registerRewardToken(address tokenAddress, string memory symbol, uint256 rewardRate) 
        external 
        onlyRole(FUND_MANAGER_ROLE) 
    {
        if (tokenAddress == address(0)) revert InvalidAmount();
        if (rewardRate == 0) revert InvalidRewardRate();
        if (rewardTokens[tokenAddress].tokenAddress != address(0)) revert DuplicateRewardToken();
     ...
     */
    function updateRewardToken(address tokenAddress, uint256 rewardRate, bool isActive) 
        external 
        onlyRole(FUND_MANAGER_ROLE) 
    {
        if (rewardTokens[tokenAddress].tokenAddress == address(0)) revert TokenNotRegistered();
        ...
        whenNotPaused 
    {
        ...
        // Verify the transfer
        if (balanceAfter - balanceBefore != amount) revert InsufficientBalance();
        ...
        nonReentrant 
    {
        ...
        // Validate stake
        if (userStake.id == 0 || userStake.id != stakeId) revert StakeNotFound();
        ...
        if (block.timestamp < userStake.endTime && !paused()) revert LockupNotCompleted();
     ...
     */
    function claimRewardsAndWithdraw(uint256 stakeId) 
        external 
        nonReentrant 
    {
        Stake storage userStake = _stakes[stakeId];
        ...
        if (kttyRewardAmount > 0) {
            ...
            emit RewardClaimed(stakeId, msg.sender, address(kttyToken), kttyRewardAmount);
        }
        ...
        for (uint256 i = 0; i < tier.rewardTokens.length; i++) {
            ...
            if (rewardTokens[tokenAddress].isActive) {
                ...
                if (tokenRewardAmount > 0) {
                    IERC20(tokenAddress).safeTransfer(msg.sender, tokenRewardAmount);
                    emit RewardClaimed(stakeId, msg.sender, tokenAddress, tokenRewardAmount);
                }
            }
        }
     ...
     */
    function hasClaimedRewards(uint256 stakeId) 
        external 
        view 
        returns (bool) 
    {
     ...
     */
    function calculateReward(uint256 stakeId, address tokenAddress) 
        external 
        view 
        returns (uint256) 
    {
        ...
        address rewardToken = tokenAddress == address(0) ? address(kttyToken) : tokenAddress;
     ...
     */
    function getTierRewardTokens(uint256 tierId) external view returns (address[] memory) {
     ...
     */
    function getUserStakes(address owner) external view returns (uint256[] memory) {
